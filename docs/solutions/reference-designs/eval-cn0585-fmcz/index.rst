...
.. figure:: setup_cn0585_diagram.png
        ...
        Development Platform Setup (note current LLDK system has CN0585 revB
        board that has USB-C power supply)
...
The :adi:`EVAL-CN0585-FMCZ <CN0585>` board has four
:adi:`ADAQ23876` 16Bit 15MSPS Data Acquisition µModules to capture
analog data from the AFE board connector. The Data Acquisition µModules are
...
0.37, 0.73, 0.87, 1.38, and 2.25, providing input voltage span ranges of ±10 V,
±5 V, ±4.096 V, ±2.5 V, and ±1.5 V. The gain/attenuation functions are
accessible through the AFE board connector pins. As an example the
:adi:`EVAL-CN0584-EBZ <CN0584>` is available for Hardware In the Loop (HIL) applications.

Transmit Channel
~~~~~~~~~~~~~~~~

The :adi:`EVAL-CN0585-FMCZ <CN0585>` board has two :adi:`AD3552R`
16-bit 33MUPS DACs that provide four analog output signals to the AFE board
...
and custom intermediate ranges with full 16-bit resolution. Refer to Table 3.
...
:adi:`LTC6655`2.048V, ±0.025% accurate, 2ppm/°C max voltage
...
.. math::
    ...
    V_(RAIL-POS) = Scale Factor * IIO Measured Voltage

    Where the scale factor is determined by the resistive divider used to monitor.
...
.. math::
    ...
    V_(RAIL-NEG) = Scale Factor * (2.5 - IIO Measured Voltage) + 2.5
...
possible to the AFE board connector. Please refer to the ADAQ23876 Datasheet for
more configuration details.

.. table:: ADQ23876 Connections for Input Configuration

+-----------------------+-----------------------+-----------------------+
| Input Range           | Input Signal on Pins  | Feedback Connections  |
+-----------------------+-----------------------+-----------------------+
| +/- 10V (Default)     | ADCx_IN2P, ADCx_IN2N  | ADCx_OUTP and         |
|                       |                       | ADCx_IN1N pins        |
|                       |                       | Shorted ADCx_OUTN and |
|                       |                       | ADCx_IN1P pins        |
|                       |                       | Shorted               |
+-----------------------+-----------------------+-----------------------+
| +/- 5V                | ADCx_IN1P, ADCx_IN1N  | ADCx_OUTP and         |
|                       |                       | ADCx_IN2N pins        |
|                       |                       | Shorted ADCx_OUTN and |
|                       |                       | ADCx_IN2P pins        |
|                       |                       | Shorted               |
...
+-----------------------+-----------------------+-----------------------+
| +/- 1.5V              | ADCx_IN1P/ADCx_IN2P   | No Connect            |
|                       | Shorted               |                       |
|                       | ADCx_IN1N/ADCx_IN2N   |                       |
|                       | Shorted               |                       |
+-----------------------+-----------------------+-----------------------+
...
output voltage range of the :adi:`AD3552R <adi>AD3552R>`__ DAC. Configuration
...
voltage of 2.5 V. The DAC current is converted to a voltage using an external
TIA. The DAC outputs are observed on signals DAC0, DAC1, DAC2, and DAC3. The
DACx outputs are fed back into the `AD3552R <adi>AD3552R>`__ gain configuration
...
refer to the `AD3552R <adi>AD3552R>`__ Datasheet for more configuration details.
...
the analog IO, ADC/DAC gain settings, GPIO, I2C, SPI, aux power, and four direct
...
Once the board is connected to the host, and power is delivered, the green LED
DS1 should be on.
...
<note important> The board can also be powered from the FMC connector by adding
resistor R9 and removing resistor R10, but it is not recommended as the current
consumption exceeds the FMC standard current limit. </note>

System Setup Using a ZedBoard
-----------------------------

The :adi:`EVAL-CN0585-FMCZ(Rev B) <CN0585>` (connected to
:adi:`EVAL-CN0584-EBZ <CN0584>`) is fully supported using a ZedBoard. For
description of system setup and functionality using
:adi:`EVAL-CN0584-EBZ <CN0584>`, :adi:`EVAL-CN0585-FMCZ <CN0585>` and
ZedBoard, refer to `CN0584 User Guide </resources/eval/user-guides/circuits-from-the-lab/cn0584>`,
...
.. admonition:: Download
    ...
    EVAL-CN0585-FMCZ Design & Integration Files `cn0585-designsupport>`__
...
- `CN0585 Circuit Note Page <ADI>CN0585>`__
- `ADAQ23876 Product Page <ADI>ADAQ23876>`__
- `AD3552R Product Page <ADI>AD3552R>`__
- `LTC6655 Product Page <ADI>LTC6655>`__
- `ADR4525 Product Page <ADI>ADR4525>`__
- `AD7291 Product Page <ADI>AD7291>`__

Reference Demos & Software
--------------------------

- `AXI_AD3552R IP core </resources/fpga/docs/axi_ad3552r>`__
- `AXI_LTC2387 IP core </resources/fpga/docs/axi_ltc2387>`__
- `AD3552R Dual Channel, 16-Bit, 33 MUPS, Multispan, Multi-IO SPI DAC Linux
  device driver <resources/tools-software/linux-drivers/iio-dac/axi-ad3552r>`__
- `LTC2387 SAR ADC IIO Linux
  Driver <resources/tools-software/linux-drivers/iio-adc/ltc2387>`__
- `High Speed Converter Toolbox for MATLAB and
  Simulink </resources/tools-software/hsx-toolbox>`__
- `PyADI-IIO Installation
  Guide </resources/tools-software/linux-software/pyadi-iio>`__
- `IIO Oscilloscope Installation
  Guide </resources/tools-software/linux-software/iio_oscilloscope>`__
- `Kuiper Linux </resources/tools-software/linux-software/kuiper-linux>`__
- `PyADI-IIO sources for the CN0585 Rev. B
  board <https://github.com/analogdevicesinc/pyadi-iio/tree/cn0585_v1>`__
- `MATLAB and Simulink sources for the CN0585 Rev. B
  board <https://github.com/analogdevicesinc/HighSpeedConverterToolbox/tree/cn0585_v1>`__
- `HDL Project sources for the CN0585 Rev. B
  board <https://github.com/analogdevicesinc/hdl/tree/cn0585_v1/projects/cn0585_fmcz>`__
- `Linux driver sources for the CN0585 Rev. B
  board <https://github.com/analogdevicesinc/linux/tree/cn0585_v1>`__
